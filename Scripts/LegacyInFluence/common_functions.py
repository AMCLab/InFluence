...
def opendatfile(image):
    ...
    datfile = np.roll(datfile, shift=0, axis=1) #change shift to shape/2 for diff patterns
    datfile = np.roll(datfile, shift=0, axis=0)
    #datfile = (datfile )/ (np.max(datfile) - np.min(datfile)) #scaling for probability map
    #datfile = datfile / np.sum(datfile)
    return datfile
