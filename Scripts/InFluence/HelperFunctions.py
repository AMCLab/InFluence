...
def CalculateConstants(ProgramParameters):                                                           # To avoid unecessary calculations within the loop but maintain flexibility, variables that are constant within the loop are calculated here and stored as class variables
        
    AlphaMultiplier = (3.4*10**-3)*(ProgramParameters.ProtonNum**0.67)                          # Where do numbers come from   #Potentially make this to a little class - maybe overkill - also performance impact if accessing lots in loop
...
ConstantsArray = np.array([AlphaMultiplier, CrossSectionNumorator, CrossSectionLogArgMultiplier, CrossSectionDenominatorA, CrossSectionDenominatorB, PathLengthMultiplier, EnergyLossMultiplierA, EnergyLossMultiplierB])
